...
def discover_kgm_toll_page_url():
    ...
    try:
        ...
        if discovered_urls:
            # Sort: prioritize page with "yeni" and latest year digits
            def sort_key(url):
                url_lower = url.lower()
                score = 0
...
def main():
    ...
    try:
        ...
        print(f"Error loading KGM page (likely temporary timeout or block): {e}")
        sys.exit(0)
    ...
    # Bu ay zaten başarılı bir güncelleme yapılmış mı kontrol edelim
    last_updated_str = data.get("metadata", {}).get("last_updated")
    if last_updated_str:
        try:
            now = datetime.datetime.utcnow()
            current_year_month = now.strftime("%Y-%m")
            if last_updated_str.startswith(current_year_month):
                print(f"Otoyol ücretleri bu ay ({current_year_month}) zaten başarıyla güncellenmiş. İşlem atlanıyor.")
                sys.exit(0)
        except Exception as e:
            print(f"last_updated kontrolü sırasında hata oluştu: {e}. İşleme devam ediliyor...")
